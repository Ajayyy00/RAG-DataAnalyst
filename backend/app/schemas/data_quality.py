...
class QualityIssue(BaseModel):
    type: str = Field(..., description="Type of issue: missing, duplicate, outlier, invalid")
    ...
    affected_ids: List[Any] = Field(default=[], description="List of primary keys affected (up to a limit)")

class AutomatedFix(BaseModel):
    description: str = Field(..., description="What the fix does")
    sql_script: str = Field(..., description="Raw SQL UPDATE/DELETE statement to fix the issue")

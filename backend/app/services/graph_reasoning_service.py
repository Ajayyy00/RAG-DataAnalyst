...
class GraphReasoningService:
    ...
    async def _reason_over_results(
        self, question: str, cypher: str, graph_data: list
    ) -> str:
    ...
    async def query(self, question: str) -> Dict[str, Any]:
        ...
        for attempt in range(max_retries):
            cypher = await self._generate_cypher(
                question, previous_error=error_msg, previous_cypher=cypher
            )
            ...
            try:
                ...
                error_msg = str(e)
                log.warning(
                    "Cypher execution failed, retrying...",
                    error=error_msg,
                    cypher=cypher,
                )
        ...
        if error_msg:
            log.error(
                "All Cypher execution attempts failed", error=error_msg, cypher=cypher
            )
            return {
                "question": question,
                "cypher": cypher,

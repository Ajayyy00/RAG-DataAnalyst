...
from app.schemas.evaluation import BenchmarkReport, EvaluationResult
...
class SQLEvaluatorService:
    ...
    def _calculate_cost(self, prompt_tokens: int, completion_tokens: int) -> float:
        return (prompt_tokens / 1000.0 * COST_PER_1K_PROMPT) + (
    ...
    async def _check_execution_accuracy(
        self, generated_sql: str, gold_sql: str
    ) -> bool:
        try:
    ...
    async def evaluate_query(
        self, benchmark_id: str, question: str, gold_sql: str
    ) -> EvaluationResult:
    ...
    async def run_benchmark(
        self, dataset_path: str = "tests/evaluation/benchmark_dataset.json"
    ) -> BenchmarkReport:
        ...
        for item in dataset:
            res = await self.evaluate_query(
                item["id"], item["question"], item["gold_sql"]
            )
            results.append(res)
        ...
        avg_latency = (
            sum(r.latency_ms for r in results) / total_queries if total_queries else 0
        )
        em_rate = (
            sum(1 for r in results if r.exact_match) / total_queries
            if total_queries
            else 0
        )

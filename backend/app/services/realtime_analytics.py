...
class RealTimeAnalyticsService:
    ...
    async def process_event(self, event_dict: dict):
        ...
        try:
            ...
            log.info("Processing realtime event", event_type=event.event_type, patient_id=event.patient_id)
            ...
            log.error("Failed to process realtime event", error=str(e), event=event_dict)

    async def _trigger_alert(self, event_type: str, severity: str, message: str, metadata: dict):
        ...
        log.info("Alert triggered and broadcasted", alert_id=str(alert.id), severity=severity)

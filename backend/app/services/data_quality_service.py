...
from app.schemas.data_quality import QualityIssue, AutomatedFix, DataQualityReport
...
class DataQualityService:
    ...
    async def analyze_table(self, table_name: str, limit: int = 10000) -> DataQualityReport:
        ...
        if not rows:
            return DataQualityReport(table_name=table_name, total_rows_analyzed=0, issues=[], automated_fixes=[])
    ...
    async def _generate_ai_fixes(self, table_name: str, issues: List[QualityIssue]) -> List[AutomatedFix]:
        if not issues:
            return []

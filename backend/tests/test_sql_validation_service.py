...
class TestValidQueries:
    def test_simple_select(self, validator):
        ...
        valid, violations, _ = validator.validate(sql)
        assert valid
        assert violations == []
...
class TestBlockedOperations:
    ...
    def test_dml_ddl_blocked(self, validator, sql):
        ...
        assert not valid
        assert any(
            "Prohibited operation" in v or "Only SELECT" in v
            for v in violations
        )
...
class TestPrecompiledPatterns:
    def test_patterns_are_precompiled(self):
        ...
        from app.services.sql_validation_service import _BLOCKED_KEYWORD_PATTERNS
        import re
        assert len(_BLOCKED_KEYWORD_PATTERNS) > 0
        for kw, pat in _BLOCKED_KEYWORD_PATTERNS.items():
